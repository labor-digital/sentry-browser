...
export default function (context: WorkerContext | CoreContext, scope: string) {
    ...
    if (scope !== 'app' && context.process !== 'worker') {
        return;
    }
    ...
    ((context: CoreContext) => {
        ...
        if (configFile === '') {
            console.log('Could not find Sentry.io config file, '
                        + 'because neither SENTRY_CONFIG_FILE_LOCATION nor BITBUCKET_CLONE_DIR are '
        ...
        context.eventEmitter.bind(AssetBuilderEventList.APPLY_EXTENSION_WEBPACK_CONFIG, (e) => {
            ...
            console.log('Injecting sentry.io configuration into the built bundle. '
                        + 'Location: process.env.LABOR_SENTRY_CONFIG');
