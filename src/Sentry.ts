...
export default class Sentry
{
    ...
    public static isActivated(): boolean
    {
        ...
        if (Sentry._manualActiveState === false) {
            return false;
        }
        if (typeof Sentry._dynamicActivityState === 'undefined') {
            Sentry._dynamicActivityState = process.env.NODE_ENV === 'production';
        }
        if (Sentry._dynamicActivityState === false && Sentry._manualActiveState !== true) {
            return false;
        }

        // Check if the class is initialized
        if (Sentry._isInitialized) {
            return true;
        }
        Sentry.initialize();
    ...
    protected static initialize()
    {
        ...
        for (let k in autoConfig) {
            if (!autoConfig.hasOwnProperty(k)) {
                continue;
            }
            if (typeof config[k] === 'undefined') {
                config[k] = autoConfig[k];
            }
        }

        // Check if we got a dsn
        if (typeof config.dsn === 'undefined') {
            return false;
        }
